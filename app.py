...
# --- 3. AI CONFIG (Optional: Jika ingin pakai API Key untuk Insight yang lebih "Liar") ---
# genai.configure(api_key="API_KEY_ANDA") # Opsional
...
try:
    ...
    # (Logika filter multiselect di sini tetap lengkap seperti script acuan)
    filters = {'TIPE_HARGA': f_thrg, 'FORM_TYPE': f_type, 'TAHUN': f_tahun, 'BULAN': f_bulan, 'VENDOR_NAME': f_vend, 'KATEGORI': f_kat, 'FSD': f_fsd, 'GROUP_NAME1': f_g1, 'GROUP_NAME2': f_g2, 'GROUP_NAME3': f_g3, 'GROUP_NAME4': f_g4}
    ...
    # (Tab 0, 1, 2 tetap dipertahankan sesuai script acuan)

    with tabs[3]: # TAB BEDAH KORELASI + INSIGHT AI
        ...
        # ... (Logika Tabel Dept Data)

        st.divider()
        st.subheader("💡 Strategic Action Plan (5W+1H)")
        
        # PROMPT DINAMIS KE AI (LOGIKA KHAZANAH INSIGHT)
        if g_lift > 200:
            ai_status = "High Impact - Whale Hunting"
            s_action = "Fokus pada Paket Bundling untuk kontraktor besar."
        elif g_corr > 0.3:
            ai_status = "Stable Growth"
            s_action = "Pertahankan loyalitas dengan program poin."
        else:
            ai_status = "Underperforming"
            s_action = "Evaluasi pemilihan SKU promo, kemungkinan tidak relevan dengan kebutuhan pasar."

        col_ins1, col_ins2 = st.columns(2)
        with col_ins1:
            st.info(f"**Dashboard Intelligence Status:**\n\nStatus: **{ai_status}**\nPearson: {g_corr:.4f}\nLift: {g_lift:.2f}%")
        with col_ins2:
            st.table(pd.DataFrame({
                "Point": ["What", "Where", "Why", "Who", "Whom", "How"],
                "Strategy": [s_action, "Showroom & Digital", "Berdasarkan data korelasi terbaru", "Sofyan (CM)", "Customer Retail", "Eksekusi via Monitoring SOQ"]
            }))

    # (Tab 4 tetap dipertahankan)
