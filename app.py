...
try:
    ...
    with tabs[3]: # TAB BEDAH KORELASI DEPT
        ...
        if not df_ds.empty:
            ...
            st.write("#### Ranking Efektivitas Promo & Benchmark")
            st.dataframe(df_ds_final.style.format({
                'Skor Pearson': '{:.4f}', 'Lompatan Omzet (%)': '{:.2f}%', 'Rata-rata Promo': 'Rp {:,.0f}'
            }), use_container_width=True)
            
            # --- BAGIAN INSIGHT 5W+1H OTOMATIS ---
            st.divider()
            st.subheader("💡 Strategic Action Plan (5W+1H)")
            
            # Logika Insight Dinamis
            if g_lift > 200 and g_corr < 0.2:
                status_promo = "⚠️ OUTLIER DETECTED"
                action_what = "Audit Transaksi Proyek/Borongan"
                action_why = f"Lompatan omzet sangat tinggi ({g_lift:.1f}%) tapi korelasi rendah ({g_corr:.4f}), artinya promo hanya dimanfaatkan segelintir pembeli besar, bukan massa."
            elif g_corr > 0.3:
                status_promo = "✅ PROMO SEHAT & STABIL"
                action_what = "Ekspansi Program Loyalitas"
                action_why = f"Korelasi {g_corr:.2f} menunjukkan harga promo sudah menjadi pola belanja yang konsisten bagi pelanggan Anda."
            else:
                status_promo = "ℹ️ MODERATE IMPACT"
                action_what = "Review Penempatan & Display Barang Promo"
                action_why = "Dampak promo ada, namun belum cukup kuat untuk menggerakkan nilai transaksi secara signifikan di seluruh departemen."

            col_ins1, col_ins2 = st.columns(2)
            with col_ins1:
                st.info(f"**Analisa Global Toko:**\n\nSaat ini korelasi Pearson Toko adalah **{g_corr:.4f}** dengan rata-rata kenaikan struk promo **{g_lift:.2f}%**.\n\nStatus: **{status_promo}**")
            ...
            with col_ins2:
                ...
                insight_table = pd.DataFrame({
                    ...
                    "Action Strategy": [
                        action_what,
                        "Area Showroom & Digital Ads",
                        action_why,
                        "Sofyan (Category Manager)",
                        "Pelanggan Retail & Kontraktor",
                        "Sesuaikan stok SOQ pada item penggerak utama (Top 5)."
                    ]
                })
                st.table(insight_table)

            fig_b = px.scatter(df_ds, x="Skor Pearson", y="Lompatan Omzet (%)", 
                              size="Jml Transaksi", color="Departemen",
                              hover_name="Departemen", text="Departemen")
            st.plotly_chart(fig_b, use_container_width=True)
        else:
            st.info("Data tidak cukup untuk komparasi departemen.")

    with tabs[4]: # TAB TOP 5 ITEMS PROMO
        ...
        if not df_p.empty:
            ...
            st.warning("Tidak ditemukan transaksi Harga Promo.")
